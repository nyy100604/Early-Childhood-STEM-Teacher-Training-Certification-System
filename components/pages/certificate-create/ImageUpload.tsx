    ...
    <>
      <div className="flex flex-row items-center gap-x-20 lg:justify-center">
        ...
        >
          ...
          />
          {image && (
            <Image src={image} fill alt="uploadImage" className="rounded-2xl" />
          )}
        </div>
        <div className="max-w-[500px] flex flex-col justify-start gap-y-8">
          ...
          <div className="w-full h-[100px] overflow-scroll bg-black/10 rounded-xl p-4 px-8">
            <p className="text-[#5F5D5D] text-[19px] ">{desc}</p>
          </div>
          {error &&
            error?.image &&
            error.image.map((error: any, index: number) => (
